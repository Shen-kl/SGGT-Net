...
def compute_group_move(group, trajectory, F, F_follower, E_follower, born_dead_set, current_time, group_center_vel, R1,
                       R2):
    """
    计算群组的运动学和状态外推。
    ...
    for group_index, group_members in enumerate(group):
        ...
        if group_state.shape[1] == 1:
            ...
            next_state = F @ trajectory[idx][:, -1].reshape(-1, 1) + vel_input_mat @ group_center_vel[
                group_index].reshape(-1, 1)
            ...
            for group_mem_index in group_members:
                ...
                if born_frame <= current_time and dead_frame >= current_time:
                    ...
                    center_pos_vel = center + np.array([[0], [0], [0], [0]])  # 4x1 (简化，因为 group_center_vel 是 2x1)
...
if __name__ == '__main__':
    ...
    group_num_truth = [2]  # 初始群组数
    target_num = 9
    frame_max = int(120 / delta_T)  # 总帧计数
    ...
    trajectory_init = [
        ...
        np.array([300, 8, 520, 0]).reshape(-1, 1),  # T5
        np.array([300, 8, 505, 0]).reshape(-1, 1),  # T6
        np.array([300, 8, 470, 0]).reshape(-1, 1),  # T7
        np.array([300, 8, 455, 0]).reshape(-1, 1),  # T8
        np.array([200, 10, 950, 2]).reshape(-1, 1)  # T9
    ]
    ...
    # 出生/消亡时间 (转换为帧数)
    born_dead_time = [
        [delta_T, 120], [delta_T, 120], [delta_T, 120], [delta_T, 106],
        [10*delta_T, 120], [10*delta_T, 120], [10*delta_T, 120], [10*delta_T, 120], [20*delta_T, 120]
    ]
    ...
    # 假设 generateLeaderFollowerTransMat 函数已导入
    # Q 在这里未使用，但在原 MATLAB 函数中有计算
    # F_follower, E_follower, F_leader, Q = generateLeaderFollowerTransMat(alpha, beta, gamma, w, delta_T)
    # 为了避免依赖，我们只关注 F_follower, E_follower, F_leader，Q可以忽略
    # F_leader 的值在 compute_group_move 中没有使用，我们使用 F_follower 和 E_follower
    # 如果需要完整的函数，请确保上一个脚本的 Python 代码已经运行或导入。
    # ------------------------------------------------------------------------------------------------
    # !!! 注意: 导入 F_follower, E_follower, F_leader 的代码需要运行上一个脚本
    # 为此，我们暂时使用占位符，但在实际运行时必须用真实值替代
    # ------------------------------------------------------------------------------------------------
    # F_follower = np.eye(4)
    # E_follower = np.eye(4)
    # F_leader = np.eye(4)
    ...
    for frame_index in range(2, frame_end_1 + 1):
        trajectory, group_num = compute_group_move(group, trajectory, F, F_follower, E_follower, born_dead_set, frame_index,
                                                   group_center_vel, R1, R2)
    ...
    # 初始化 group_center_vel 以匹配 group 列表大小
    group_center_vel_stage3 = [
        np.array([0, 0]),
        np.array([0, 0]),
        np.array([0, 0]),
        np.array([0, 0])
    ...
    for frame_index in range(frame_start_3, frame_end_3 + 1):
        if frame_index == frame_start_3:
            # 138/delta_T + 1 帧
            group_center_vel_stage3[0] = np.array([0, 0])
            group_center_vel_stage3[1] = np.array([0, 1])
            group_center_vel_stage3[2] = np.array([0, -1])
            group_center_vel_stage3[3] = np.array([0, 0])
            # MATLAB 中的 group_center_vel{5} 不存在，已忽略

        elif frame_index == frame_start_3 + 2:  # 138/delta_T + 3 帧
            group_center_vel_stage3[0] = np.array([0, 0])
            group_center_vel_stage3[1] = np.array([0, 2])
            group_center_vel_stage3[2] = np.array([0, -2])
            group_center_vel_stage3[3] = np.array([0, 0])
            # MATLAB 中的 group_center_vel{5} 不存在，已忽略
