...
class MotionModelBase(nn.Module):
    ...
    def state_transition_matrix(self, X, inp, static_f):
        ...
        fx = lambda state, inputs, static_input: solvers['rk4'](self.model_update, state,
                                                               inputs, static_input, self.dt)
        jacobian_rev = vmap(jacrev(fx, argnums=0), randomness='same')(X.flatten(0, 1), inp.flatten(0, 1), static_f.flatten(0, 1))
        F = jacobian_rev.view(N, m, feat_dim, feat_dim)
        return F, torch.transpose(F, dim0=-2, dim1=-1)

    def input_transition_matrix(self, X, u):
        batch_size = X.size(0)
        G = nn.Parameter(self.build_inp_transition_matrix(), requires_grad=False)
        G = G.clone().expand(batch_size, -1, -1, -1)
...
# class SecondOrderNeuralODE_groupTrack(MotionModelBase):
#     """
#         x = vx
#         y = vy
#         vx, vy = NN(vx, vy, u1, u2)
...
#                  n_hidden=32, n_layers=2, u1_lim=10, u2_lim=10, u3_lim=10, u4_lim=10, u5_lim=10, u6_lim=10, u7_lim=10, u8_lim=10):
#         super().__init__(solver, dt, n_states, mixtures, static_f_dim, u1_lim, u2_lim,
#                          u3_lim, u4_lim, u5_lim, u6_lim, u7_lim, u8_lim)
#         # n_inputs = 2
#         self.n_states = n_states
#         self.f = nn.ModuleList([self.create_net(n_states , 3 + self.static_f_dim,
#                                                 n_hidden, n_layers) for _ in range(n_states // 2)])
#
#         # x, y
#         self.m0 = nn.Parameter(torch.zeros(n_states, n_states // 2), requires_grad=False)
...
#
#         # vy
#         self.m1 = nn.Parameter(torch.zeros(10, 7), requires_grad=False)
#         self.m1[0, 0] = self.m1[1, 1] = self.m1[2, 2] = self.m1[3, 3] = self.m1[4, 4] = self.m1[6, 5] = self.m1[8, 6] = 1.
#
#         # vx
#         self.m2 = nn.Parameter(torch.zeros(10, 7), requires_grad=False)
#         self.m2[0, 0] = self.m2[1, 1] = self.m2[2, 2] = self.m2[3, 3] = self.m2[5, 4] = self.m2[7, 5] = self.m2[9, 6] = 1.
#
#         self.config = CONFIG
#
#         # self.self_dynamics = ResMLP(4,32,2, layer_num=2, dropout_rate=0.1)
#
...
#         u_acc = u[..., -2:]
#         u_struct_acc = u[..., -4:-2]
#         inp = torch.cat((X, u_x), dim=-1)
#
#         acc = self.backbone(inp)
#
#         dvx = acc[...,0].unsqueeze(-1) + u_acc[...,0].unsqueeze(-1) + u_struct_acc[...,0].unsqueeze(-1)
#         dvy = acc[...,1].unsqueeze(-1) + u_acc[...,1].unsqueeze(-1) + u_struct_acc[...,1].unsqueeze(-1)
#
#
#         scale = self.config['S_VEL'] / self.config['S_POS']
#         dxdt = scale * (X @ self.m0)
#
#         dX = torch.cat((dxdt, dvx, dvy), dim=-1)
#         return dX

# class SecondOrderNeuralODE_groupTrack_without_struct(MotionModelBase):
#     """
#         x = vx
#         y = vy
#         vx, vy = NN(vx, vy, u1, u2)
...
#         dvx = acc[...,0].unsqueeze(-1) + u_acc[...,0].unsqueeze(-1)
#         dvy = acc[...,1].unsqueeze(-1) + u_acc[...,1].unsqueeze(-1)
...
# class SecondOrderNeuralODE_groupTrack_without_MCU(MotionModelBase):
#     """
#         x = vx
#         y = vy
#         vx, vy = NN(vx, vy, u1, u2)
...
#         dvx = acc[...,0].unsqueeze(-1) + u_struct_acc[...,0].unsqueeze(-1)
#         dvy = acc[...,1].unsqueeze(-1) + u_struct_acc[...,1].unsqueeze(-1)
...
# class SecondOrderNeuralODE_groupTrack_without_struct_MCU(MotionModelBase):
#     """
#         x = vx
#         y = vy
#         vx, vy = NN(vx, vy, u1, u2)
